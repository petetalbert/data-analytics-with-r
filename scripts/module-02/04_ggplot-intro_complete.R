...
ggplot(office_ratings_summary, aes(x = season, y = avg_total_votes)) + 
  geom_col()
...
ggplot(office_ratings, aes(x = season)) + 
  geom_bar()
# the data is just a count of seasons. geom_col() forces you to supply a y-axis value; geom_bar() just counts the number
# of rows from the raw data, kind of like the count() function!
