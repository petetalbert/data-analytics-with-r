...
office_ratings_new <- mutate(office_ratings, imdb_rating_binned = case_when(imdb_rating < 8 ~ "Less than 8",
                                                                            imdb_rating >= 8 & imdb_rating < 9 ~ "Between 8 and 9",
                                                                            imdb_rating >= 9 ~ "Greater than 9"))
...
ggplot(office_ratings_new, aes(x = season, fill = imdb_rating_binned)) +
  geom_bar()

# 3) USE mutate() and fct_relevel() TO CHANGE THE ORDER OF THE VALUES IN YOUR NEW COLUMN.
#    MAKE SURE TO SAVE IT AGAIN.
office_ratings_new <- mutate(office_ratings_new, imdb_rating_binned = fct_relevel(imdb_rating_binned, "Less than 8", "Between 8 and 9", "Greater than 9"))
...
office_ratings_new <- mutate(office_ratings_new, air_date_month = month(air_date, label = TRUE))

# 6) USE COUNT TO DETERMINE WHAT THE MOST COMMON air_date MONTH WAS FOR THE SERIES. DON'T FORGET TO SORT!

count(office_ratings_new, air_date_month, sort = TRUE)
