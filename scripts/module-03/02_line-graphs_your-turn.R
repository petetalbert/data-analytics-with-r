...
ggplot(office_ratings, aes(x = ___, y = ___)) +
  geom_line()


# 2) MAKE THE SAME PLOT BUT SUBSITUTE air_date FOR epsiode.
ggplot(office_ratings, aes(x = ___, y = imdb_rating)) +
  geom_line()
...
ggplot(office_ratings, aes(x = ___, y = imdb_rating, color = as_factor(___))) +
  geom_line()
...
___ <- filter(office_ratings, imdb_rating >= ___ & imdb_rating <= ___ & total_votes >= ___ & total_votes <= ___)

ggplot(___, aes(x = total_votes, y = imdb_rating)) +
  geom_point()
